...
def db_connection(details):
    ...
    try:
        ...
        el.error_log("Database connection", e)
        return None
...
def db_insert(table_name, columns):
    ...
    try:
        ...
        if not isinstance(columns, list):
            el.error_log("db_insert()", "Columns must be a list of column names.")
            return None
        ...
        el.error_log(f"Load phase error generating INSERT statement for table {table_name}.", f"{e}\n{Error}")
        return None